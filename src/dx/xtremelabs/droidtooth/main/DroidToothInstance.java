...
/**
 * The all-knowing DroidToothInstance singleton which deals with all low-level Bluetooth functionality.
 * This class is a core component of this library and can be used externally outside of DroidTooth class.
 * 
...
public class DroidToothInstance {
	...
	//various Bluetooth state listeners for when BT is turning on/ff, is on/off, 
	//started/stopped discovery, and found device while scanning.
	private BroadcastReceiver bluetoothStateListener, deviceFoundListener, discoveryStateListener;
	...
	/**
	 * To be called first thing in the morning! Then subsequent calls to getInstance() 
	 * are guaranteed to provide a non-null instance. This avoids the null-checker statement
	 * every time getInstance() is called. 
	 * @param activity main activity onto which DroidTooth will register listeners.
	 */
	...
	/**
	 * If initInstance(Activity) is called first, this method will always return an instance
	 * of DroidToothInstance which contains a variety of mid-low level Bluetooth functions.
	 * The assumption is that at this level of access, the developer is responsible for
	 * having called initInstance() before attempting to get an instance.
	 *  
	...
	/**
	 * Obviously, initialize the listeners for the tooth.
	 * Each listener handles their corresponding tasks associated with 
	 * the various event triggers of the Droid Ttooth (Bluetooth).
	 * 
	 *  To entirely change the way any of DroidTooth's listeners work, simply extend one of the abstract listeners 
	 *  in dx.xtremelabs.droidtooth.abstract_listeners, and override the onReceive() method, but be sure to call super 
	 *  on it so that DroidTooth is also made aware of those events as they happen.
	 *  
	 */
	private void initToothListeners(){
		//listen for whenever the state of Bluetooth changes (turning on, on, turning off, off) 
		...
		//finally make sure we are made aware whenever a device is found while scanning in progress
	...
	/**
	 * If required to re-initialize existing listeners after a call unregisterListeners()
	 * call this method to put the listeners in the android system.  
	 */
	public void registerListeners(){
		activity.registerReceiver(bluetoothStateListener, new IntentFilter(BluetoothAdapter.ACTION_STATE_CHANGED));
		activity.registerReceiver(discoveryStateListener, new IntentFilter(BluetoothAdapter.ACTION_DISCOVERY_STARTED));
		activity.registerReceiver(discoveryStateListener, new IntentFilter(BluetoothAdapter.ACTION_DISCOVERY_FINISHED));
		activity.registerReceiver(deviceFoundListener, new IntentFilter(BluetoothDevice.ACTION_FOUND));
	...
	/**
	 * Return the registered listener DTBluetoothStateListener 
	 * which listens for when Bluetooth is turning on, is on,
	 * turning off, is off.
	...
	/**
	 * Return the registered listener DTDeviceFoundListener
	 * which listens and takes action for when a new device 
	...
	/**
	 * Return the registered listener DTDiscoveryStateListener
	 * which listens and takes action for when discovery starts and stops.
	 ...
	 * simply riding on the parent abstract classes and implement your own functionality
	 * rather than issuing callbacks to interested clients. 
	 * @param the associated listeners cast up to parent BroadcastReceiver objects.
	 */
	
	public void setBluetoothStateListener(BroadcastReceiver bluetoothStateListener) {
	...
	public void setDiscoveryStateListener(BroadcastReceiver discoveryStateListener) {
		this.discoveryStateListener = discoveryStateListener;
	}
	/********************************************************************/
	
	/**
	 * Return a new DroidToothServer instance thread used to handle incoming connections.
	 * @return
	 */
	 ...
	 * See android.bluetooth.IBluetooth*.aidl
	...
	public IBluetooth getIBluetooth() {
		...
		try {
			...
			java.lang.reflect.Method m2 = c2.getDeclaredMethod("getService",String.class);
		    android.os.IBinder b = (android.os.IBinder) m2.invoke(null, "bluetooth");
		    ...
		    Class c = s2[0];
			java.lang.reflect.Method m = c.getDeclaredMethod("asInterface",android.os.IBinder.class);
	...
	public void setOnDeviceFoundCallback(DTCallback callback){
		((DTDeviceFoundListener)deviceFoundListener).setOnDeviceFoundCallback(callback);
	}
	public void setDiscoveryStartedCallback(DTCallback callback){
		((DTDiscoveryStateListener) discoveryStateListener).setDiscoveryStartedCallback(callback);
	}
	...
	/**
	 * Initialize the Bluetooth adapter for use. If Bluetooth is not ON, it will be turned on
	 * and BluetoothStateListener will known of the BT state.
	 * 
	...
	public boolean initBluetooth(){
		...
		//ensure Bluetooth is enabled, at which point BluetoothStateListener would know what to do.
		if (!droidTooth.isEnabled()){
			droidTooth.enable(); 
		} else {
			bluetoothPreviouslyOn = true; //Bluetooth device is already ON, keep this state for when we're done.
		}
	...
	/**
	 * Start the discovery process of any nearby visible devices.
	 * Return whether discovery was possible. Prevent attempts to 
	 * issue a re-discover if device is already discovering. It is up
	 * to the developer to utilize the DiscoveryStateListener to know
	 * when to issue the next available time for discovery, if required.
	 * 
	 * @param forceRestart whether to stop current discovery (if present) and start discovery again.
	 * @return 	DISCOVERY_STARTED if the discovery course is able to carry through.
	 * 			DISCOVERY_ADAPTER_NULL if adapter is null.
	 * 			DISCOVERY_ALREADY_ONGOING if discovery is already.
	 * 			DISCOVERY_RESTARTED if forced to stop an existing discovery process and started anew.
	 */
	private int startDiscovery(boolean forceRestart){
		...
		if (droidTooth.isDiscovering()){ //if device is currently scanning 
			...
			} else { //cancel any existing discovery process if asked to force restart
	...
	/**
	 * Try to start discovering for devices. Does not intrude in any existing discovery processes.
	 ...
	 * @return whether discovery started for the first time or DISCOVERY_RESTARTED (forced).
	 */
	...
	/**
	 * Try and turn off the Bluetooth device.
	 * If param is passed is True, it will restore the state of Bluetooth as it was
	 * prior to DroidTooth interfacing with the BT device. Also, receivers will not be 
	 * unregistered from the main activity. If gracefully = False, then any 
	 * discovery process will be cancelled, BT listeners will be unregistered 
	 * and the BT finally off.
	 * 
	 ...
	 * @param gracefully toggle whether to turn off Bluetooth gracefully
	...
	/**
	 * Try and turn off Bluetooth if the device state was originally off
	 * prior to DroidTooth enabling it. If Bluetooth was ON when DroidTooth
	 * launched, then leave it on for the user to decide.
	 * 
	 * @return  whether Bluetooth was turned off, if BT was on before DroidTooth interfaced with it, returns True. 
	 */
	...
	/**
	 * Start an intent to prompt the user to make this device discoverable
	 * for the default amount of seconds.
	 */
	 ...
	 * @param durationInSeconds integer value for the duration of being discoverable, 
	 * 						if 0 then the default discovery duration is used.
	 */
	public void forceBecomeDiscoverable(int durationInSeconds, boolean forced){
		...
		if (forced && discoverabilityRunner.isRunning()){
			...
			getDActivity().updateConsole("Executing thread for discoverability...");
	...
	/**
	 * At the moment, the only way that Bluetooth stops being discoverable 
	 * is if the discovery time's up. Thus, we simply change visibility to end within 1 second.
	 * However to do this, we need permission of the user to turn on bluetooth discoverability
	 * for that 1 second... which is not a very user-friendly experience. 
	 * 
